...
exports.checkOverdueTickets = onSchedule("* * * * *", async (event) => {
  try {
    ...
    // 1. 使用 collectionGroup 穿透抓取設定檔 (相容 Canvas 與正式環境)
    ...
    console.log(`目前逾期標準：${overdueHours} 小時`);
    ...
    if (!LINE_ACCESS_TOKEN) {
      console.error("錯誤：尚未設定 LINE_ACCESS_TOKEN");
      return;
    }
    ...
    for (const ticket of overdueTickets) {
      const targetUser = ticket.assignee || ticket.receiver; // 優先找指定處理人，沒有則找建檔人
    ...
    if (pushCount > 0) {
      await batch.commit();
      console.log(`成功發送 ${pushCount} 筆通知！`);
    }

  } catch (error) {
    console.error("排程發生錯誤:", error);
  }
});
